...
#define VOLTAGE_THRESHOLD 2.50f
void opticalRoutineSend()
{
	for(unsigned char i = 0; i < ADC_INPUT_COUNT; i++)
	{
		unsigned short int adcData = analog[i];
		if (adcData < VOLTAGE_THRESHOLD*400)
		{
			...
			buff[3] = (char)i;
			buff[4] = 0;
			sendData(mainPort, buff, adcData);
...
{
	PORTB = 0xFF;
	PORTD = (1<<PORT3) | (1<<PORT2) | (1<<PORT1) | (1<<PORT0);
	PORTE = (1<<PORT7) | (1<<PORT6) | (1<<PORT5) | (1<<PORT4);
	PORTJ = 0xFF;
